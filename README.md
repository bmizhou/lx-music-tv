...
- **自动切换平台**：可以设置是否开启js源播放失败切换到其他平台进行尝试
...
- **扫码推送搜索文字**：搜索页二维码按钮 → 手机/电脑扫码打开 `http://电视IP:5777/search` 页面，输入文字一键推送到 TV 搜索框（解决遥控器输入困难，支持清空按钮；仅在搜索页生效）
...
| 二维码 | ZXing core 3.5.3（搜索页扫码推送搜索文字，本地生成二维码） |
...
### 问题反馈  
2.9版本设置页面加入了web端查看日志的功能，如果遇到什么问题需要提issue可以附带上这部分日志。
